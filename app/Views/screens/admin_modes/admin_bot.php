...
?>
<div class="admin-card mb-20"  style="border: 1.5px solid #8b5a2b; background: rgba(139, 90, 43, 0.03);">
    ...
    <div  class="mt-15" style="display: flex; gap: 20px; flex-wrap: wrap;">
        <!-- Summon Form -->
        <div  style="flex: 1; min-width: 250px; background: rgba(0,0,0,0.03); padding: 12px; border-radius: 4px; border: 1px solid rgba(139,90,43,0.15);">
            <form method="post" action="<?= $adminBaseUrl ?>&mode=bot&action=summon_bots">
                <label  class="bold" style="display: block; margin-bottom: 8px;">Invocador de Bots Vikings:</label>
        ...
        <div  style="flex: 1; min-width: 250px; background: rgba(0,0,0,0.03); padding: 12px; border-radius: 4px; border: 1px solid rgba(139,90,43,0.15); display: flex; flex-direction: column; justify-content: center;">
            <span  class="bold" style="display: block; margin-bottom: 8px;">Processar Turno da IA:</span>
            <form method="post" action="<?= $adminBaseUrl ?>&mode=bot&action=run_bots_now">
                <button type="submit" class="btn w-100"  style="background: #2e7d32; border-color: #1b5e20; color: white; padding: 8px;" <?= !$isDiamond ? 'disabled' : '' ?>>
    ...
    <?php if (!$isDiamond): ?>
        <p  class="bold mt-10" style="color: #b8860b; font-size: 11px;"><i class="fas fa-lock"></i> Atualize o seu servidor para a Licença Diamond para libertar o poder da IA em massa!</p>
    <?php endif; ?>
</div>
