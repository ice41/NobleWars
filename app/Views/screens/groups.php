...
<?php if (!empty($error)): ?>
    <div class="error_box p-10 mb-15 bold"  style="background-color: #ffd2d2; border: 1px solid #d29191; color: #a02020; border-radius: 4px;">
        <?= htmlspecialchars($error) ?>
    </div>
<?php endif; ?>

<?php if (!empty($success)): ?>
    <div class="success_box p-10 mb-15 bold"  style="background-color: #d2ffd2; border: 1px solid #91d291; color: #20a020; border-radius: 4px;">
    ...
    <form action="game.php?village=<?= $village['id'] ?>&amp;screen=groups_bar&amp;mode=edit&amp;id=<?= $group['id'] ?>&amp;action=save&amp;h=<?= htmlspecialchars($hkey ?? '') ?>" method="post">
        <table class="vis" width="100%">
            <thead>
                <tr>
                    <th width="30">
                        <input type="checkbox" onclick="toggleAllVillages(this);"  class="pointer" />
                    </th>
                    <th>Aldeia</th>
                    <th>Grupo Atual</th>
                </tr>
                ...
                <?php else: ?>
                    <?php foreach ($villages as $v): ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($is_in_group): ?>
                                    <span  class="text-green bold">Este Grupo</span>
        ...
        <br />
        <input type="submit" value="Guardar Alterações" class="btn bold pointer"  style="padding: 5px 15px;" />
    </form>
...
<?php else: ?>
    ...
    <table class="vis" width="100%">
        <thead>
            <tr>
                <th>Nome do Grupo</th>
                <th width="150"  class="text-center">Quantidade de Aldeias</th>
        ...
        <tbody>
            <?php if (empty($groups)): ?>
                <tr>
                    <td colspan="3" align="center"  class="p-10">Não tem nenhum grupo criado. Crie um grupo abaixo para começar a organizar as suas aldeias!</td>
                </tr>
            <?php else: ?>
    ...
    <div class="vis"  style="padding: 15px; border: 1px solid #dfd1af; background-color: #f7eed3;">
        ...
        <form action="game.php?village=<?= $village['id'] ?>&amp;screen=groups_bar&amp;action=create&amp;h=<?= htmlspecialchars($hkey ?? '') ?>" method="post">
            ...
            <input type="text" id="group_name" name="name" maxlength="30" placeholder="ex: ataque, defesa, mista..." required  class="p-5" style="width: 250px; margin-left: 10px;" />
            <input type="submit" value="Criar Grupo" class="btn bold pointer"  style="padding: 5px 15px; margin-left: 10px;" />
        </form>
    </div>
<?php endif; ?>
